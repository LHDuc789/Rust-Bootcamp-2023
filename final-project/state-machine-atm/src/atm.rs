    ...
    fn next_state(starting_state: &Self::State, t: &Self::Transition) -> Self::State {
        match &t {
            &Action::SwipeCard(pin_hash) => {
                ...
                else if starting_state.expected_pin_hash == Auth::Authenticating(*pin_hash) {
                    if starting_state.keystroke_register.is_empty() {
                        Atm {
                            cash_inside: starting_state.cash_inside,
                            expected_pin_hash: Auth::Authenticating(*pin_hash),
                            keystroke_register: Vec::new(),
            ...
            &Action::PressKey(key) => {
                if key == &Key::Enter {
                    if starting_state.expected_pin_hash == Auth::Authenticated {
                        if starting_state.keystroke_register.len() == 1{
                            Atm {
                        ...
                        let ss = starting_state.keystroke_register.clone();
                        let pin_hash = crate::traits::hash(&ss);
                        if starting_state.expected_pin_hash != Auth::Authenticating(pin_hash) {
                            Atm {
                                cash_inside: starting_state.cash_inside,
                                expected_pin_hash: Auth::default(),
                                keystroke_register: Vec::new(),
                ...
                else if starting_state.expected_pin_hash == Auth::default() {
                    Atm {
                        cash_inside: starting_state.cash_inside,
                        expected_pin_hash: Auth::default(),
                        keystroke_register: Vec::new(),
                ...
                else if starting_state.expected_pin_hash == Auth::Authenticating(1234) {

                    if starting_state.keystroke_register.is_empty() {
                        Atm {
                            cash_inside: starting_state.cash_inside,
                            expected_pin_hash: starting_state.expected_pin_hash.clone(),
                            keystroke_register: vec![key.clone()],
                        }
                    }

                    else{
                        let k = starting_state.keystroke_register.clone();
                        let mut store_k: Vec<Key> = Vec::new();
                        for i in k {
                            store_k.push(i.clone());
                        }
                        store_k.push(key.clone());
                        Atm {
                            cash_inside: starting_state.cash_inside,
                            expected_pin_hash: starting_state.expected_pin_hash.clone(),
                            keystroke_register: store_k.clone(),
                        }
                    }
